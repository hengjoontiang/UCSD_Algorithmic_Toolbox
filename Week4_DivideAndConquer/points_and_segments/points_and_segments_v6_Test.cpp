...
/*
 * The data structure whereby we store ranges to check number of interscetions does nt work when
 * the point coincides with the edge of line segments
 * 
 * eg 
 * starts[0]=-5;ends[0]=10;
	...
	By existing, it doesnt cater for situation when point coincide wif boundary, say 5 --> # intersections = 5
	 -5 ---------------------------------10
	 -5 --------------------------9
	 -5 --------------5
	                  5--6
	...
	for point =5, the number of intersections by the naive method = 5
	* 
	* 
	* 
	* To cater for this, we will retain the original starts,ends , without the use of a special data structure
	* instead we will have a moving window  into both line segments , and the points
	* 1. sort starts/ends , intertwined, by the starts , in increasing order
	* 2. find min of starts and max of ends
	* 3. create an index to starts/ends pair
	* 4. init pt_map , the count line segment = 0 for all points
	* 5. create a copy of points, and placed them into a set (i.e no repeat)
	* 6. drop points in which < min of starts
	* 7. drop points in which > max of ends
	* 8. iterate through remaining points, 
	* 9.			for each point:
	* 					point_score = 0
	* 					if current_point == last point : UpdateOrInsertMap(current_point,last_score)
	* 10.				iterate through all the ranges that the point lies in:
	* 11.					for each range:
	* 12.						point_score++
	* 13.				UpdateOrInsertMap(point,score)
	* 14.				Move the index of range to next range
	* 15.			    last_point = current_point;last_score = point_score		 
 ...
 * */
 ...
 int main() {
	...
	*/
	...
	//1. obtain the min of starts
	
	//2. obtain the max of ends
	
	//3. sort the input points
