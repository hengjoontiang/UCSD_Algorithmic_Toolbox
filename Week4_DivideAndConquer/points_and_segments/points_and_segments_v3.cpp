...
std::vector<int>  fast_count_segments_v2(vector<int> starts, vector<int> ends, vector<int> points) {
  std::map<int,int>  cnt; //must not init size
  ...
  for (size_t i = 0; i < points.size(); i++) {
	...
	if (aux_point != -1) { //proceed to update the number of intersections
		std::pair<int,int> p = std::make_pair(current_point,aux[aux_point].second);
		cnt.insert(p);
	}else {
		std::pair<int,int> p= std::make_pair(current_point,0);
		cnt.insert(p);
	}
  }
  
  std::vector<int> return_cnt(points.size());
  for (size_t i = 0; i < points.size(); i++) {
...
int main() {
  ...
  */
  ...
  std::vector<int>  cnt = fast_count_segments_v2(starts, ends, points);
