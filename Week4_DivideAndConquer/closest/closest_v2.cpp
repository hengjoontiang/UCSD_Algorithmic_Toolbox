...
double FindClosestDistanceByBruteForce(vector<Point> pointVec){
	...
	for (long i=0; i< pointVec.size();i++) {
		...
		for (long j=i+1; j< pointVec.size();j++) {
			...
			//shortcut
			if (leftPoint.x == rightPoint.x && leftPoint.y == rightPoint.y ) return 0;
...
double findClosestDistanceFromVectWithinD(vector<Point> dminToMidFromLeftAndRightVec, double shortestDist) {
	...
	for (long index_p1_i = 0; index_p1_i < lenVect; index_p1_i++) {
		for (long index_p2_i = index_p1_i+1; index_p2_i < lenVect; index_p2_i++) {
			...
			//alsready sorting in ascending order, can break out now
			if (yDist >=  shortestDist) break;
...
double FindClosestDistance(const vector<Point> & pointVec){
	...
	for (long dIndex_Dec = midIndex; dIndex_Dec >=0  ; dIndex_Dec --) {
		long abs_xDistMidVsCurr_Dec = xDistMid - pointVec[dIndex_Dec].x ;
		if (abs_xDistMidVsCurr_Dec <= dMin_Left_Right) { 
			dminToMidFromLeftAndRightVec.push_back(pointVec[dIndex_Dec]);
		}else break;
	}
	for (long dIndex_Inc = midIndex+1; dIndex_Inc < len_pointVec  ; dIndex_Inc ++) {
		long abs_xDistMidVsCurr_Inc = pointVec[dIndex_Inc].x - xDistMid;
		if (abs_xDistMidVsCurr_Inc <= dMin_Left_Right) { 
			dminToMidFromLeftAndRightVec.push_back(pointVec[dIndex_Inc]);
		}else break;
	}
